...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def Plotting2dim(self, data, Signal, Background_Top, Background_diboson, Background_W, Background_Z, Background_MiltiJet, Indice, channel, lepton, var, multi, Energy, bandUncer):
        ...
        # Create a figure and axes
        fig, (ax1, ax2) = aplt.ratio_plot(name="fig1", figsize=(1600, 800), hspace=0.05)
        ...
        if "enu" in channel:
            Binning = [0,  0,  0,  0,  0,  0,  0,  0,  0, 2.47, 0,  0,  0,  0,  0,  0,  0,  0,  0, 2.47, 0,  0,  0,  0,  0,  0,  0,  0,  0,
                      2.47, 0,  0,  0,  0,  0,  0,  0,  0,  0, 2.47, 0,  0,  0,  0,  0,  0,  0,  0,  0, 2.47, 0,  0,  0,  0,  0,  0,  0,  0,  0, 2.47]
            xlabel = [0., 0.1, 0.6, 0.8, 1.15, 1.37, 1.52, 1.81, 2.01, 2.37, 2.47, 2.57, 3.07, 3.27, 3.62, 3.84, 3.99, 4.28, 4.48, 4.84, 4.94, 5.04, 5.54, 5.74, 6.09, 6.31, 6.46, 6.75, 6.95, 7.31, 7.41, 7.51,
            ...
            RecoBin   = [0., 0.1, 0.46, 0.66, 0.95, 1.1, 1.32, 1.67, 1.87, 2.37, 2.47, 2.57, 3.07, 3.27, 3.62, 3.84, 3.99, 4.28, 4.48, 4.84, 4.94, 5.04, 5.4, 5.6, 5.89, 6.04, 6.26, 6.61, 6.81, 7.31, 7.41, 7.51,
        ...
        if "munu" in channel:
            Binning = [0,  0,  0,  0,  0,  0, 2.4,  0,  0,  0,  0,  0,  0, 2.4,  0,  0,  0,  0,  0,  0,
                      2.4,  0,  0,  0,  0,  0,  0, 2.4,  0,  0,  0,  0,  0,  0, 2.4,  0,  0,  0,  0,  0,  0, 2.4]
            xlabel = [0., 0.482, 1.052, 1.2521, 1.35, 1.492, 1.924, 2.4, 2.876, 3.308, 3.45, 3.5479, 3.748, 4.318, 4.8, 5.282, 5.852, 6.0521, 6.15, 6.292, 6.724, 7.2,
        ...
        data_hist   = TH1F("data_hist", "data_hist", len(RecoBin)-1, array('d', RecoBin))  # data
        sig_hist    = TH1F("sig_hist",  "sig_hist",  len(RecoBin)-1, array('d', RecoBin))  # Signal
        bkg_hist1   = TH1F("bkg_hist1", "bkg_hist1", len(RecoBin)-1, array('d', RecoBin))  # Top
        bkg_hist2   = TH1F("bkg_hist2", "bkg_hist2", len(RecoBin)-1, array('d', RecoBin))  # diboson
        bkg_hist3   = TH1F("bkg_hist3", "bkg_hist3", len(RecoBin)-1, array('d', RecoBin))  # diboson
        bkg_hist4   = TH1F("bkg_hist4", "bkg_hist4", len(RecoBin)-1, array('d', RecoBin))  # diboson
        bkg_hist5   = TH1F("bkg_hist5", "bkg_hist5", len(RecoBin)-1, array('d', RecoBin))  # diboson
        histbg      = TH1F("histbg",    "histbg",    len(RecoBin)-1, array('d', RecoBin))  # diboson
        ...
        LineWidth = 1
        bkg_hist1.SetLineWidth(LineWidth)
        bkg_hist2.SetLineWidth(LineWidth)
        bkg_hist3.SetLineWidth(LineWidth)
        bkg_hist4.SetLineWidth(LineWidth)
        bkg_hist5.SetLineWidth(LineWidth)
        sig_hist.SetLineWidth(LineWidth)

        bkg_hist1.SetFillColor(root.kRed+1)
        bkg_hist2.SetFillColor(root.kGreen+1)
        bkg_hist3.SetFillColor(root.kMagenta+2)
        bkg_hist4.SetFillColor(root.kOrange+7)
        bkg_hist5.SetFillColor(root.kOrange+134)
        sig_hist.SetFillColor(root.kAzure+1)
        # Draw the stacked histogram on these axes
        ax1.plot(bkg_and_sig)
        #bkg_and_sig.GetYaxis().SetLabelSize(2)
        line1.Draw("same")
        line2.Draw("same")
        line3.Draw("same")
        line4.Draw("same")
        ...
        # Plot the MC stat error as a hatched band
        err_band = aplt.root_helpers.hist_to_graph(
            bkg_and_sig.GetStack().Last(), show_bin_width=True)
        ax1.set_yscale("log")
        #ax1.plot(err_band, "2", fillcolor=1, fillstyle=3254, linewidth=0)

        # Plot the data as a graph
        ax1.set_ylim(1, 3000000)
        data_graph = aplt.root_helpers.hist_to_graph(data_hist)
        ax1.plot(data_graph, "P")
        ...
        errgraph = root.TGraphErrors(len(x), array('d', x), array(
            'd', y), array('d', errx), array('d', erry))
        xax1 = errgraph.GetXaxis()
        for i in range(0, len(RecoBin)):
            xax1.SetBinLabel(i+1, str(33))

        ROOT.gPad.Modified()
        ROOT.gPad.Update()
        #ax2.plot(errgraph, "2", fillcolor=1, fillstyle=3254)
        #ax2.plot(errgraph, "2", fillcolor=1, fillstyle=3002)
        ax2.plot(errgraph, "2", fillcolor=1, fillstyle=3005)
        #ax2.plot(errgraph, "2", fillcolor=1, fillstyle=3001)
        #ax2.plot(errgraph, "2", fillcolor=1, fillstyle=4000)
        #ax2.plot(errgraph, "2", fillcolor=1, fillstyle=4050)

        # Calculate and draw the ratio
        ratio_hist = data_hist.Clone("ratio_hist")
        ratio_hist.Divide(bkg_and_sig.GetStack().Last())
        for i in range(0, ratio_hist.GetNbinsX()):
            if ratio_hist.GetBinContent(i+1) == 0:
                ratio_hist.SetBinContent(i+1, 1)
        ratio_graph = aplt.root_helpers.hist_to_graph(ratio_hist)
        ax2.plot(ratio_graph, "P0")
        ...
        # Set axis titles
        ax2.set_xlabel("\eta^{l}")
        ax1.set_ylabel("Events", titleoffset=1.2)
        ax2.set_ylabel("Data / Pred.", loc="centre", titleoffset=1.2)
        ax2.set_ylim(0.89, 1.11)
        ax2.draw_arrows_outside_range(ratio_graph)

        if "munu" in channel:
            line11 = TLine(2.4,  0.89,    2.4,   1.11)
            line22 = TLine(4.8,  0.89,    4.8,   1.11)
            line33 = TLine(7.2,  0.89,    7.2,   1.11)
            line44 = TLine(9.6,  0.89,    9.6,   1.11)
            line55 = TLine(12,   0.89,    12,    1.11)
            line66 = TLine(14.4, 0.89,    14.4,  1.11)

        if "enu" in channel:
            line11 = TLine(2.47,  0.89,   2.47, 1.11)
            line22 = TLine(4.94,  0.89,   4.94, 1.11)
            line33 = TLine(7.41,  0.89,   7.41, 1.11)
            line44 = TLine(9.88,  0.89,   9.88, 1.11)
            line55 = TLine(12.35, 0.89,  12.35, 1.11)
            line66 = TLine(14.82, 0.89,  14.82, 1.11)
        ...
        # Go back to top axes to add label6
        ax1.cd()
        ...
        if "munu" in channel:
            ax1.text(0.20,  0.66, "25<p^{l}_{T}<30",   size=24, align=13)
            ax1.text(0.32,  0.66, "30<p^{l}_{T}<35",   size=24, align=13)
            ax1.text(0.45,  0.66, "35<p^{l}_{T}<40",   size=24, align=13)
            ax1.text(0.58,  0.66, "40<p^{l}_{T}<45",   size=24, align=13)
            ax1.text(0.72,  0.66, "45<p^{l}_{T}<50",   size=24, align=13)
            ax1.text(0.845, 0.66, "50<p^{l}_{T}<100",  size=24, align=13)
        if "enu" in channel:
            ax1.text(0.20,  0.66, "25<p^{l}_{T}<30",   size=24, align=13)
            ax1.text(0.32,  0.66, "30<p^{l}_{T}<35",   size=24, align=13)
            ax1.text(0.45,  0.66, "35<p^{l}_{T}<40",   size=24, align=13)
        ...
        # Add legend
        legend = ax1.legend(loc=(0.6, 0.72, 1 - root.gPad.GetRightMargin() -
                                 0.03, 1 - root.gPad.GetTopMargin() - 0.03), textsize=22, ncol=4)
        ...
        fig.savefig("Output/Backgrounds/Lepton_vs_Eta_""_" +Energy+"_"+channel+".pdf")
